...
export default {
  ...
  computed: {
    ...
    uploadUrl() {
      ...
      return this.pageType ? `http://192.168.3.84:8088/api/plan/user-${this.pageType.toLowerCase()}/${projectId}` : '';
    },
    defaultData() {
    return {
      ERD: {
        sampleUrl: 'https://www.erdcloud.com/',
        imagePreview: 'http://192.168.3.84:8088/img/plan/default-erd-image.png'
      },
      USECASE: {
        sampleUrl: 'https://darw.io/',
        imagePreview: 'http://192.168.3.84:8088/img/plan/default-usecase-image.png'
      },
      ui: {
        sampleUrl: 'https://www.figma.com/',
        imagePreview: 'http://192.168.3.84:8088/img/plan/default-ui-image.png'
      }
    };
  }
  },
  ...
  methods: {
    ...
    axios.delete(`http://192.168.3.84:8088/api/plan/${pageTitle}`)
        .then(response => {
            Swal.fire({
                title: 'Success!',
                text: '페이지가 삭제되었습니다.',
  ...
  axios.post('http://192.168.3.84:8088/api/plan/create-page', formData, {
    headers: {
...
saveUrl() {
  ...
  axios.put(`http://192.168.3.84:8088/api/plan/update-url/${projectId}/${title}`, { newSampleUrl: this.editableSampleUrl })
    .then(response => {
      Swal.fire({
      title: 'Success!',
      text: 'URL 수정 성공!',
  ...
  if (this.pageType) {
    ...
    if (this.pageType && projectId) {
      const url = `http://192.168.3.84:8088/api/plan/${this.pageType.toLowerCase()}-example/${projectId}`;
      axios.get(url)
        .then(response => {
