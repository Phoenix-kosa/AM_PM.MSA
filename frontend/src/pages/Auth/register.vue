...
const checkUserId = () => {
    ...
    axios.get(`http://localhost:8088/api/user/user_id/${user_id}`)
        .then(response => {
            if (response.data) {
...
let checkEmail = () => {
    ...
    axios.get(`http://localhost:8088/api/user/email/${email}`)
        .then(response => {
            if (response.data) {
...
const submitForm = () => {
    if(emailavaliable & idavailable){
        axios.post('http://localhost:8088/api/user', formData)
        .then(response => {
